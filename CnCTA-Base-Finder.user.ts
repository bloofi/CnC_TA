            ...
            const Main = qx.Class.define('Main', {
                ...
                members: <ClassMembers>{
                    ...
                    refreshWindow: function() {
                        ...
                        const totalPlayersFetched = (Object.values(this.players) as PlayerItem[]).filter(m => m.isFetched).length;
                        const totalBases = Object.keys(this.bases).length;
                        const totalBasesFetched = (Object.values(this.bases) as BaseItem[]).filter(m => m.isFetched).length;
                        const totalGhosts = (Object.values(this.bases) as BaseItem[]).filter(m => m.isFetched && m.g).length;
                        if (totalBases > 0) {
                            this.fetchLabel.set({
                                value: [
                    ...
                    refreshSelect: function() {
                        ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(
                            ...
                            phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this.onRankingGetData),
                            null,
                        );
                    },
                    ...
                    onSelectAlliance: function() {
                        const selectA: AllianceItem = this.allianceSelect.getModelSelection().getItem(0);
                        this.resetAlliance();
                        ...
                        if (selectA && selectA.id > 0) {
                            ...
                            ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(
                                ...
                                phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this.onGetPublicAllianceInfo),
                                null,
                            );
                        } else {
                    ...
                    onButtonFetchAlliance: function() {
                        const customAlliance: string = this.allianceTextfield.getValue();
                        this.resetAlliance();
                        if (customAlliance && customAlliance !== '') {
                            ...
                            ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(
                                ...
                                phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this.onGetPublicAllianceInfoByNameOrAbbreviation),
                                null,
                            );
                        } else {
                    ...
                    onButtonShowGhosts: function() {
                        this.showMarkers((Object.values(this.bases) as BaseItem[]).filter(b => b.g));
                    },

                    onButtonShowMains: function() {
                        this.showMarkers((Object.values(this.bases) as BaseItem[]).filter(b => b.isMain));
                    },
                    ...
                    onGetPublicAllianceInfoByNameOrAbbreviation: function(context: any, data: AllianceInfo) {
                        if (data && data.i) {
                    ...
                    onGetPublicAllianceInfo: function(context: any, data: AllianceInfo) {
                        this.updateAllianceInfo(data);
                    },

                    onRankingGetData: function(context: any, data: RankingInfo) {
                        this.allianceSelect.removeAll();
                        this.allianceSelect.add(new qx.ui.form.ListItem('', null, { id: 0, name: '' } as AllianceItem));
                        this.favorites.forEach((a: AllianceItem) => {
                            this.allianceSelect.add(new qx.ui.form.ListItem(`[fav] ${a.name}`, null, a));
                        });
                        data.a.forEach((a, i) => {
                            this.allianceSelect.add(new qx.ui.form.ListItem(`${i + 1} - ${a.an}`, null, { id: a.a, name: a.an } as AllianceItem));
                        });
                    },

                    onGetPublicPlayerInfo: function(context: any, data: PlayerInfo) {
                        if (data && data.c) {
                            const idMain: number = data.c.reduce((p, c) => (c.p > p.p ? c : p), data.c[0]).i;
                            this.players[`pid-${data.i}`] = {
                                ...this.players[`pid-${data.i}`],
                                c: data.c.map(cc => ({ ...cc, pn: data.n, isMain: cc.i === idMain, isGhost: null })),
                                isFetched: true,
                            };
                            data.c.forEach(b => {
                                this.bases[`b-${b.i}`] = {
                                ...
                                ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(
                                    ...
                                    phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this.onGetPublicCityInfoById),
                                    null,
                                );
                            });
                            this.refreshWindow();
                    ...
                    onGetPublicCityInfoById: function(context: any, data: CityInfo) {
                        if (data && data.i) {
                            this.bases[`b-${data.i}`] = {
                                ...this.bases[`b-${data.i}`],
                                ...data,
                    ...
                    updateAllianceInfo: function(data: AllianceInfo) {
                        ...
                        data.m
                            ...
                            .forEach(m => {
                                this.players[`pid-${m.i}`] = { ...m, fetched: false };
                                ClientLib.Net.CommunicationManager.GetInstance().SendSimpleCommand(
                                    'GetPublicPlayerInfo',
                                    {
                                        id: m.i,
                                    },
                                    phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, this.onGetPublicPlayerInfo),
                                    null,
                                );
                            });
                        this.refreshWindow();
                    ...
                    showMarkers: function(bases: BaseItem[]) {
                    ...
                    addMarker: function(base: BaseItem, text: string) {
                        ...
                        marker.add(
                            new qx.ui.basic.Label('' + text).set({
                                textColor: '#ffffff',
                                opacity: 1,
                    ...
                    removeMarkers: function() {
                        (Object.values(this.bases) as BaseItem[]).forEach(b => {
                            if (b.marker) {
                                qx.core.Init.getApplication()
                                    .getDesktop()
                                    .remove(b.marker);
                    ...
                    repositionMarkers: function() {
                        this.updateMarkerSize();
                        (Object.values(this.bases) as BaseItem[])
                            .filter(b => b.marker)
                            .forEach(b => {
                    ...
                    loadStorage: function() {
                        ...
                        this.favorites =
                            storage[
                                `wid-${ClientLib.Data.MainData.GetInstance()
                                    .get_Server()
                                    .get_WorldId()}`
                            ] || [];
                    },
