...
(function() {
    type ObjType = 'NONE' | 'CAMP' | 'OUTPOST' | 'BASE' | 'PLAYER' | 'GHOST';
    type ScannerFetchStatus = 'WAITING' | 'FETCHING' | 'FETCHED' | 'CANCELED';
    type ScannerStatus = 'READY' | 'SCANNING' | 'FETCHING' | 'END';
    type ScannerFilters = {
        scanCamps: boolean;
        scanOutposts: boolean;
        scanBases: boolean;
        scanPlayers: boolean;
        scanFrom: 'all' | number;
    };
    ...
    type ScannerStorage = {
        ...
        scanFilters?: ScannerFilters;
        cache?: { [xy: string]: string };
    };
    type Members = {
        scanStatus: ScannerStatus;
        scanResults: ScannerResult[];
        storage: ScannerStorage;
        bases: { [id: string]: ScannerResult };
        scanIds: number[];
            ...
            const Main = qx.Class.define('Main', {
                ...
                members: <Members>{
                        ...
                        switch (this.scanStatus) {
                            ...
                            case 'FETCHING':
                                if (this.currentScanID) {
                                    const b: ScannerResult = this.bases[this.currentScanID];
                    ...
                    filterResults: function() {
                        ...
                        (Object.values(this.bases) as ScannerResult[]).forEach(sr => {
                            if (sr.status === 'FETCHED') {
                    ...
                    checkAndFetch: function() {
                        if (this.scanStatus === 'FETCHING') {
                            if (this.currentScanID) {
                                this.refreshLabel();
                                const currentScan: ScannerResult = this.bases[this.currentScanID];
                                switch (currentScan.status) {
                                    case 'WAITING':
                                        ...
                                        if (this.storage.cache[`${this.currentWid}-${currentScan.x}:${currentScan.y}`]) {
                                            currentScan.layout = this.cncoptToLayout(
                                                this.storage.cache[`${this.currentWid}-${currentScan.x}:${currentScan.y}`],
                                            );
                    ...
                    findNext: function() {
                        const nextBase = (Object.values(this.bases) as ScannerResult[]).find(b => b.status === 'WAITING');
                        if (nextBase) {
                    ...
                    openCncLV: function(sr: ScannerResult) {
                        return function() {
                    ...
                    computeScores: function(layout: string[][]) {
                        const resTib = {
                            t6: 0,
                            t5: 0,
                            t4: 0,
                    ...
                    loadStorage: function() {
                        this.storage = { ...defaultStorage, ...(JSON.parse(localStorage.getItem(storageKey) || '{}') || {}) };
                    },

                    saveToStorage: function(data: ScannerStorage) {
                        this.storage = {
                            ...this.storage,
                            ...(data || {}),
                        };
